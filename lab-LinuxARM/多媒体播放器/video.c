...
int videoplay(){
    char sys_cmd[50];
    sprintf(sys_cmd,"mplayer %s -zoom -x 800 -y 480",name[num]);
    //sprintf(sys_cmd,"mplayer %s -zoom -x 800 -y 480 </dev/null>/dev/null2>1&",name[num]);
    system(sys_cmd);
}
    ...
    while(1){
        ...
        if(ev.type==EV_KEY&&ev.code==BTN_TOUCH){
            if(ev.value==0){
                ...
                if(xtou>250&&xtou<500){
                    last();
                }
                if(xtou>500&&xtou<750){
                    next();
                }
                if(xtou>750&&xtou<1000){
                    returnmain();
                }
            }
        }
